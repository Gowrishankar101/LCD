...
void setup()
{
  ...
  for(int i=0;i<10;i++)            // print elint[] array
  {
    lcd.print(elint[i]);
    delay(400);
  }
  ...
  for(int g=0;g<3;g++)
   ...
   delay(500);                     // wait for 500ms
    ...
    for(int i=0;i<8;i++)           // print firm[] array
    {
    lcd.print(firm[i]);
    delay(400);
    }
    ...
    for(int i=0;i<4;i++)           // print version[] array
    {
    lcd.print(Version[i]);
    delay(400);
    }
  ...
  */
  ...
  lcd.clear();                       // clear the LCD screen
  lcd.blink();                       // enable blinking block cursor
}
  ...
  if (Serial.available() > 1)                                // check if data has been sent from the computer
     {
       ...
       howmany = Serial.available();                         // check howmany bytes are available in serial buffer for reading
       letters = howmany;                                    // assign 'howmany' variable to another variable
       ...
       while(l<42)                     
        ...
        NL = 4;                                              // set NL variable value to 4
  for(int z=4;z<40;z++)                                      // check for message terminate characters
  {
    if(cmd[z] == ' ' || cmd[z] == '\n' || cmd[z] == '\r')
    {
      goto skip;                                             // transfer control to address 'skip'
    }
  NL++;                                                      // increment NL variable
  }
skip:  
  

// This block of code is used for message printing
i=a=0;                                                       // assigning variables
while(a<3)                                                   // check for print command
{
if(cmd[a] == pri[a])
{
i++;                                                         // increment i
...
{
i=0;                                                         // assign 0 to i                                  
Serial.println("Printed");                                   // send acknowledgement
for(int f=4;f<NL;f++)                                        // print message on lcd
   {
     lcd.print(cmd[f]); 
     delay(20);
   }
